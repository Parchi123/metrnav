...
# Optional import for speech recognition (not always available)
try:
...
def load_metro_data():
    ...
    # Ensure all station names and lines are strings to avoid unexpected types
    for station in list(stations_data.keys()):
        stations_data[str(station)]["lines"] = [str(line) for line in stations_data[station]["lines"]]

    # (G already created above) return consolidated data
    return {
...
def voice_to_text():
    # Guard if speech_recognition package is not available
    if not SPEECH_RECOGNITION_AVAILABLE:
        st.error("Speech recognition package not installed or unavailable on this system.")
        return None
...
def sidebar_interface():
    ...
    nav_option = st.sidebar.selectbox(
        ...
        ["Route Planner", "Metro Map & Live Updates", "Station Finder", 
         "Traffic Analysis", "User Routes", "Metro Chatbot"]
    )
...
def route_planner_interface(stations, current_time):
    ...
    if planning_mode == "Time-Based":
        # Pass a `time` object as default to `st.time_input`
        departure_time = st.time_input("Departure Time:", current_time.time())
